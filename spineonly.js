class plotto {
  ...
  get svgPathPoints() {
    return this.#svgPathPoints;
  }
  set svgPathPoints(v) {
    this.#svgPathPoints = Array.isArray(v) ? v : [];
  }

  get SVG_BOX_SIZE() {
    return this.#SVG_BOX_SIZE;
  }
  set SVG_BOX_SIZE(v) {
  ...
  set link2(v) {
    this.#link2 = Number(v) || 0;
  }

  get upperRestAngle() {
    return this.#upperRestAngle;
  }
  set upperRestAngle(v) {
    this.#upperRestAngle = Number(v) || 0;
  }

  get foreRestAngle() {
    return this.#foreRestAngle;
  }
  set foreRestAngle(v) {
    this.#foreRestAngle = Number(v) || 0;
  }

  get JOINT2_OFFSET() {
    return this.#JOINT2_OFFSET;
  }
  set JOINT2_OFFSET(v) {
    this.#JOINT2_OFFSET = Number(v) || 0;
  }
  ...
  buildMotionJsonFromSvg() {
    ...
    const moveToTarget = (targetJ1, targetJ2, penState) => {
      ...
      while (rem1 !== 0 || rem2 !== 0) {
        const d1 = Math.max(
          -this.MAX_STEPS_PT,
          Math.min(this.MAX_STEPS_PT, rem1)
        );
        const d2 = Math.max(
          -this.MAX_STEPS_PT,
          Math.min(this.MAX_STEPS_PT, rem2)
        );
    ...
    for (let idx = 0; idx < totalPoints; idx++) {
      ...
      const ik = this.inverseKinematics2DOF(pt.x, pt.y, prevJ1Deg, prevJ2Deg);
    ...
    console.log("plotEncode?", typeof plotEncode);
    console.log("encodeDeltaByte?", typeof encodeDeltaByte);
    try {
    ...
    while ((match = re.exec(transformStr)) !== null) {
      ...
      if (name === "translate") {
        ...
        if (typeof cx === "number" && typeof cy === "number") {
          ...
          m = multiplyMatrices(
            T(cx, cy),
            multiplyMatrices(R(ang), T(-cx, -cy))
          );
        } else {
          m = R(ang);
        }
...
function resamplePathByAngle(points, maxDeltaDeg = plotto.MAX_DELTA_DEG) {
  ...
  function subdivide(p0, ik0, p1, depth = 0) {
    if (depth > 20) {
      const ik1_fallback =
        plotto.inverseKinematics2DOF(p1.x, p1.y, ik0.joint1, ik0.joint2) || ik0;
      return [{ point: p1, ik: ik1_fallback }];
    }

    const ik1 = plotto.inverseKinematics2DOF(
      p1.x,
      p1.y,
      ik0.joint1,
      ik0.joint2
    ...
    const ikMid = plotto.inverseKinematics2DOF(
      mid.x,
      mid.y,
      ik0.joint1,
      ik0.joint2
