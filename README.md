...
본 문서는 로봇팔 시뮬레이터 프로젝트 **Plutto**에서 정의한 하드웨어 통신용 최소 바이트 데이터 형식인 **Plutto Path**에 대한 정의 및 변환 방식을 설명합니다.

Plutto Path는 SVG 경로 파싱 후 생성되는 로봇팔 관절 움직임 정보(JSON)를 **1바이트(8비트) 크기의 배열**로 압축하여, 하드웨어(로봇팔 컨트롤러)와의 효율적인 시리얼 통신을 목적으로 합니다.

## 2. Plutto Path 자료형 구조

Plutto Path는 **1바이트(8비트)** 크기의 숫자를 원소로 하는 **1차원 배열** 형태의 자료형입니다. 배열의 각 원소(1바이트)는 다음 두 가지 유형 중 하나를 나타냅니다.

1.  **동작 명령 (Delta Byte):** 두 관절의 움직임 변화량 ($d1$, $d2$)을 동시에 인코딩.
2.  **펜 상태 변경 명령 (Control Byte):** 펜의 Up/Down 상태 변경을 알림.

### 2.1. 동작 명령 (Delta Byte) 구조

동작 명령 바이트는 두 관절의 움직임 변화량($d1, d2$)을 담고 있습니다. 1바이트(8비트)는 정확히 두 개의 4비트 니블(Nibble)로 나뉘어 다음과 같이 관절 변화량을 할당합니다.

* **상위 4비트 (High Nibble):** 관절 1의 스텝 변화량 ($d1$)을 인코딩합니다.
* **하위 4비트 (Low Nibble):** 관절 2의 스텝 변화량 ($d2$)을 인코딩합니다.

$$( \text{Byte} = (d1 \text{ Nibble} \ll 4) \mid d2 \text{ Nibble} )$$

### 2.2. 펜 상태 변경 명령 (Control Byte)

움직임 변화량 인코딩에 사용되지 않는 특정 바이트 값을 **펜 상태 변경을 위한 제어 명령**으로 예약합니다. 이는 $d1$ 또는 $d2$의 4비트 인코딩 시 **$0b1000$ (8)** 값이 사용되지 않는다는 점을 활용합니다.
...
2.  **동작 명령 변환:** $d1$과 $d2$ 값을 4비트 인코딩 맵을 사용하여 각각 `high` Nibble과 `low` Nibble로 변환합니다.
3.  **1바이트 구성:** $(high \ll 4) \mid low$ 연산을 통해 $d1$과 $d2$를 담은 1바이트 **Delta Byte**를 배열에 추가합니다.
...
3.  **동작 명령 해석:**
    * $B$가 제어 명령이 아닌 경우, $d1$ (상위 4비트)과 $d2$ (하위 4비트)를 4비트 디코딩 맵을 사용하여 역변환합니다.
