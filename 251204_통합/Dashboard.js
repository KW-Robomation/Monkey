...
function dashboard() {
  ...
  J2.append("text").attr("x", 15).attr("y", 70).attr("font-size", "12px").text("J2 (deg)");
  createSlider(J2, [sliderWidth, sliderX, 60], "angle_J2", 0, 120, 0);

  // J1 슬라이더
  const J1 = frame.append("g");
  J1.append("text").attr("x", 15).attr("y", 115).attr("font-size", "12px").text("J1 (deg)");
  createSlider(J1, [sliderWidth, sliderX, 105], "angle_J1", -120, 120, 0);

  // 속도 슬라이더
  const speed = frame.append("g");
  speed.append("text").attr("x", 15).attr("y", 160).attr("font-size", "12px").text("Speed");
  createSlider(speed, [sliderWidth, sliderX, 150], "angle_speed", 0, 100, 100);

  // Pen Up/Down 버튼
  const penBtn = frame.append("g");
  createButton(penBtn, [100, 50, 200], "pen_toggle_btn", "Pen Up/Down");
